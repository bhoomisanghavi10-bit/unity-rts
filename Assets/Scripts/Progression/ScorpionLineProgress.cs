...
namespace KingdomsOfBharat.Progression
{
    ...
    // ladder - Bana Yantra (base) -> Maha Bana Yantra. Only 2 tiers total,
    // gated at Durg/Imperial per the roadmap's own item text - mirrors
    // CamelRiderLineProgress's/CavalryArcherLineProgress's shape exactly
    // (the closest existing precedent for a 2-tier Durg/Imperial line).
    // Tier 0's RequiredAge is descriptive only, never enforced (same
    // convention every other line's own RequestTrain* already established
    // - RequestTrainScorpion has no age gate of its own). Tier 1's
    // bonus/cost reuses every other line's own established Imperial-gate
    // growth exactly (+30 HP/+6 dmg/200 Gold/100 Wood/40s) - the closest
    // existing curve at a matching gate, not independently balanced.
    public readonly struct ScorpionTierData
    {
        ...
        public ScorpionTierData(string name, AgeId requiredAge, float hpBonus, float damageBonus, float goldCost, float woodCost, float researchTime)
        {
            Name = name;
    ...
    public static class ScorpionLineProgress
    {
        // Tier 0 (Bana Yantra) is the current flat Scorpion - trainable
        // with no research, so it carries no cost/research-time entry of
        // its own (never looked up as a "next tier" to research).
        public static readonly ScorpionTierData[] Tiers =
        {
            new ScorpionTierData("Bana Yantra", AgeId.Durg, hpBonus: 0f, damageBonus: 0f, goldCost: 0f, woodCost: 0f, researchTime: 0f),
            new ScorpionTierData("Maha Bana Yantra", AgeId.Imperial, hpBonus: 30f, damageBonus: 6f, goldCost: 200f, woodCost: 100f, researchTime: 40f),
        };
        ...
        public static ScorpionTierData Current(FactionId faction) => Tiers[Tier(faction)];

        public static bool HasNextTier(FactionId faction) => Tier(faction) < MaxTier;

        public static ScorpionTierData NextTierData(FactionId faction) => Tiers[Tier(faction) + 1];
