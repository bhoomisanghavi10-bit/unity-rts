...
{
    // Creates a Lumber Camp foundation (LumberCamp + ConstructionSite +
    // FactionMember). Mirrors HouseFactory/FarmFactory's shape - used by
    // BuildingPlacer for the Player's mouse-driven placement.
    public static class LumberCampFactory
    {
        ...
        public static GameObject Place(Vector3 point, FactionId faction, float buildTime)
        {
            ...
            GameObject go = BuildingModelFactory.Spawn("LumberCamp", civ, point + Vector3.up * (Size.y * 0.5f), Size, profile.PrimaryColor, faction: faction);
            go.name = faction == FactionId.Player ? "LumberCamp" : "EnemyLumberCamp";
            BuildingFootprint.Attach(go, BuildingFootprint.Square(BuildingFootprint.DropOffTiles), carveObstacle: true);

            go.AddComponent<LumberCamp>();
